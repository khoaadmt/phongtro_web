...
import * as user_historyService from '../services/user_history'
    ...
    try {
        ...
        const detector = new DeviceDetector({
          ...
          if(result.device.brand != ""){
            var detailDevice = result.device.brand + " " + result.device.model
          }else{
            var detailDevice = result.os.name + " " + result.device.type
          }

        if(userId != ""){
            await user_historyService.createNewUser_historyService(userId,"login","from " +detailDevice)
        }
        return res.status(200).json(response)
